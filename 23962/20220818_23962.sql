...
VALUES(-55, 'TestБазовая сумма минус процент филиала от базовой суммы, если по ДТ валютная касса', 'groovy', 'OPER_LIM', 'return doc.cashDecrease & doc.creditSumma.foreignCurrency ? baseSum.subtract(baseSum.multiply(service.getParamService().getParamValueGlobalNumber("LIMITATION_SALE_PERCENT").divide(100))) : null', 'BASE_SUM_MINUS_LSP');

INSERT INTO SP.EXPR
(EXPR_ID, EXPR_NAME, EXPR_TYPE_CODE, EXPR_SCOPE_CODE, EXPR_TEXT, EXPR_CODE)
VALUES(-66, 'TestПроцент филиала от базовой суммы, если по ДТ валютная касса', 'groovy', 'OPER_LIM', 'return doc.cashDecrease & doc.creditSumma.foreignCurrency ? baseSum.multiply(service.getParamService().getParamValueGlobalNumber("LIMITATION_SALE_PERCENT").divide(100)) : null', 'BASE_SUM_LSP');
...
update SP.OPER_LIM_OPERATION_GROUP
set
SUB_DELTA_EXPR_ID = '-66'
where OPER_LIM_ID = '-2'
and OPERATION_GROUP_CODE = '-26';

update SP.OPER_LIM_OPERATION_GROUP
set
SUB_DELTA_EXPR_ID = '-55'
where OPER_LIM_ID = '-1'
and OPERATION_GROUP_CODE = '-26';
